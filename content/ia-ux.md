...
archetype (**required** **automatic**)
: The archetype is link to the archetype JSON file.
  This value is automatically generated by the system, and cannot be
...
: In many circumstances our system will need to know what type an item
  ...
  "image/jpeg"), because the item\_type can imply something about the role of
  the item as well as its format (is that image a company logo, or a profile
  photo?).

attribution
: We use the attribution field to ensure proper attribution. This field is not
...
category
: The category string is a keyword-optimized string that will be used to
  calculate the canonical URL of the item, as the "path". Generally this should
  indicate the subject of the item. If multiple categories are provided, the
  first will be used in the canonical URL.
...
slug (**required**)
: The slug field is a keyword-optimized string that will be used to calculate
  the canonical URL for the item, as the "file name". If not provided, one will
  be generated from the title.

thumbnail
: The thumbnail field is a link to an image asset that can be used to represent
  this item in a visual context (the way the title is used to represent it in a
  textual context).
